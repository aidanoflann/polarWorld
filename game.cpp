	...
	while( !quit_state )
	{
		...
		if (state == Running)
		{
			...
			if ( cloud->doSpawn(d_time, player->getNumKills()) )
			{
				...
				int coin = rand()%4;
				if (coin == 0)
						enemies.push_back( new GroundEnemy( cloud->getR(), cloud->getTheta(), 1, std::min(player->getNumKills()*0.001+0.02, 0.1)) );
				else if (coin == 1)
						enemies.push_back( new GroundEnemy( cloud->getR(), cloud->getTheta(), -1, std::min(player->getNumKills()*0.001+0.02, 0.1)) );
				else if (coin == 2)
						enemies.push_back( new SkyEnemy( cloud->getR(), cloud->getTheta(), 1, std::min(player->getNumKills()*0.001+0.02, 0.1)) );
				else if (coin == 3)
						enemies.push_back( new SkyEnemy( cloud->getR(), cloud->getTheta(), -1, std::min(player->getNumKills()*0.001+0.02, 0.1)) );
			}
			//tick all bullets
